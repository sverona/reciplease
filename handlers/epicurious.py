...
class EpicuriousHandler(RecipeHandler):
    ...
    sites = {"epicurious.com": "Epicurious"}

    # I swear to fuck...
    def title(self) -> str:
        tag = self.page.soup.find("h1", {"data-testid": "ContentHeaderHed"})
        return text(tag)

    def author(self) -> str:
        tag = self.page.soup.find("span", {"data-testid": "BylineName"})
        return re.sub("^By ", "", text(tag))

    def yield_(self) -> str:
        tag = self.page.soup.find(class_=re.compile("^Yield.*"))
        return re.sub("^Makes ", "", text(tag))

    def ingredients(self) -> SubheadingGroup:
        container = self.page.soup.find(
    ...
    def instructions(self) -> SubheadingGroup:
        ...
        sections = self.page.soup.find_all(
    ...
    def time(self) -> dict[str, str]:
        ...
        containers = self.page.soup.find_all(
            class_=re.compile("^InfoSliceItem.*")
        )
